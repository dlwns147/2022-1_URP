...
training_condition_path = '/media/data1/sangjun/VFP290K_label'
train_image_paths = sorted(glob.glob(os.path.join(data_root_dir, "*/images/*.jpg")))
train_label_paths = sorted(glob.glob(os.path.join(data_root_dir, "*/clean_xml/*.xml")))
...
for image_path, label_path in tqdm(zip(train_image_paths, train_label_paths), total=len(train_image_paths), leave=False):
    ...
    dir_path = os.path.join(training_condition_path, label_path.split("/")[-3])
    if not os.path.exists(dir_path) :
        os.mkdir(dir_path)
    with open(os.path.join(dir_path, label_path.split("/")[-1].split(".")[0]+".txt"), 'w') as f :
        f.write(ret_string)
